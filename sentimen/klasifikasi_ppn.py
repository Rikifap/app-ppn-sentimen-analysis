...
default_file_path = "ppn_labeled_and_embedded.csv" # Pastikan nama file ini ada di direktori yang sama
...
# Pastikan X_train dan y_train sudah terdefinisi di sini dari load_and_split_data()
...
# Latih model ini dengan data train Anda agar bisa digunakan di tab4
best_model_gnb_from_colab.fit(X_train, y_train)
...
# Latih model ini dengan data train Anda agar bisa digunakan di tab4
best_model_svm_from_colab.fit(X_train, y_train)
...
st.markdown(
    "<h1 class='main-title'>Visualisasi Hasil Klasifikasi Terkait Pajak Pertambahan Nilai (PPN) <br> Menggunakan Algoritma <span style='color:#1560BD;'>Naive Bayes Classifier dan Support Vector Machine (SVM)</span></h1>",
    unsafe_allow_html=True
)
...
with tab1:
    ...
    with col_split_info:
        ...
        # 1. Ubah warna latar belakang figure dan axes agar sesuai tema gelap
        fig_test_bar.patch.set_facecolor('#0E1117') 
        ax_test_bar.set_facecolor('#1E1E1E')
        
        sns.countplot(x=y_test_labels, ax=ax_test_bar, palette='viridis')
        
        # 2. Ubah semua warna teks dan garis (spines) dari 'black' menjadi 'white'
        ax_test_bar.set_title(f'Distribusi Label Data Testing (Total: {test_count})', color='white', fontsize=12)
        ax_test_bar.set_xlabel('')
        ax_test_bar.set_ylabel('Jumlah Data', color='white')
        ax_test_bar.tick_params(axis='x', colors='white')
        ax_test_bar.tick_params(axis='y', colors='white')
        ax_test_bar.spines['bottom'].set_color('white')
        ax_test_bar.spines['left'].set_color('white')
        ax_test_bar.spines['top'].set_visible(False)
        ax_test_bar.spines['right'].set_visible(False)
        
        # 3. Ubah warna teks anotasi pada bar menjadi 'white'
...
with tab3:
    ...
    with col_gnb_metrics:
        ...
        precision_gnb_train = precision_score(y_train, y_pred_gnb_train, average='weighted', zero_division=0)
        recall_gnb_train = recall_score(y_train, y_pred_gnb_train, average='weighted', zero_division=0)
        f1_gnb_train = f1_score(y_train, y_pred_gnb_train, average='weighted', zero_division=0)

        # Prediksi untuk data uji GNB
    ...
    with col_svm_metrics: # Kolom kedua untuk SVM
        ...
        # Prediksi untuk data latih SVM
        y_pred_svm_train = best_model_svm.predict(X_train)
        accuracy_svm_train = accuracy_score(y_train, y_pred_svm_train)
        precision_svm_train = precision_score(y_train, y_pred_svm_train, average='weighted', zero_division=0)
        recall_svm_train = recall_score(y_train, y_pred_svm_train, average='weighted', zero_division=0)
        f1_svm_train = f1_score(y_train, y_pred_svm_train, average='weighted', zero_division=0)

        # Prediksi untuk data uji SVM
        ...
        # --- Tampilkan Tabel SVM ---
        st.subheader("Tabel Metrik Support Vector Machine (SVM)") # Subheader untuk tabel
        data_svm_table = {
            'Metrik': ['Akurasi', 'Presisi', 'Recall', 'F1-Score'],
        ...
        st.subheader("Diagram Performa Support Vector Machine (SVM)") # Subheader untuk diagram
...
with tab4:
    ...
    with col_svm_eval:
        ...
        st.write(f"**Akurasi (SVM): `{accuracy_score(y_test, y_pred_svm):.4f}`**")
